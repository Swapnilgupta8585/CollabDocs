...
func (h *Handler) HandleUpdateUserCredentials(w http.ResponseWriter, r *http.Request){
	...
	err = h.Cfg.Db.AddHashPassword(r.Context(), database.AddHashPasswordParams{HashedPassword: hashPassword, ID: userId})
	if err != nil {
	...
	err = h.Cfg.Db.UpdateEmailOfUser(r.Context(), database.UpdateEmailOfUserParams{Email: param.Email, ID: userId})
	if err != nil{
	...
	user, err := h.Cfg.Db.GetUserByEmail(r.Context(), param.Email)
	if err != nil {
