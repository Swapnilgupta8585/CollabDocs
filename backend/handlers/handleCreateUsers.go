...
func (h *Handler) HandleCreateUsers(w http.ResponseWriter, r *http.Request) {
	...
	user, err := h.Cfg.Db.CreateUser(r.Context(), reqParam.Email)
	if err != nil {
	...
	err = h.Cfg.Db.AddHashPassword(r.Context(), database.AddHashPasswordParams{HashedPassword: hashedPassword, ID: user.ID})
	if err != nil {
